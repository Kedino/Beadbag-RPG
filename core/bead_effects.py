...
def add_scaled_effect(entity, effect_name, amount=1):
    effect = copy.deepcopy(EFFECTS[effect_name])
...
def bead_effect_vulnerability(entity, *args, amount=1, **kwargs):
    effect_types = ["physical_vulnerability", "magical_vulnerability"]
    for effect_name in effect_types:
        add_scaled_effect(entity, effect_name, amount)

def bead_effect_resilience(entity, *args, amount=1, **kwargs):
    effect_types = ["physical_resilience", "magical_resilience"]
    for effect_name in effect_types:
        add_scaled_effect(entity, effect_name, amount)
